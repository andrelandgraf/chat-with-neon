...
- **Profile pictures.** Users upload an avatar to the function's JWT-gated `/avatar` route (stored in Object Storage under a stable per-user key, persisted to a `profiles` table). The function broadcasts a `profile` event so every client updates that user's avatar live, next to all of their messages (looked up client-side by user id).
